import { ForwardRefExoticComponent, type HTMLAttributes, type ReactNode } from "react";
...
export declare const buttonSlots2: SlotComponents<{
    base: HTMLButtonElement;
}>;
type CreateVariant<Slots extends Record<string, (props: Prettify<HTMLAttributes<HTMLElement> & {
    slot: string;
}>) => JSX.Element>> = <Args, ReturnType extends Partial<Record<keyof Slots, string>>>(cb: (slots: Slots, args: Args) => ReturnType) => (args: Args) => ReturnType;
...
type SlotComponents<Slots extends Record<string, HTMLElement>> = {
    [SlotName in keyof Slots]: ForwardRefExoticComponent<Prettify<HTMLAttributes<Slots[SlotName]> & React.RefAttributes<Slots[SlotName]> & {
        __Element?: SlotName;
    }>>;
};
export {};
